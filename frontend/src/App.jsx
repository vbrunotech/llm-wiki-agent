...
export default function App() {
  ...
  const handleWikiLink = useCallback(slug => {
    ...
    const page = wikiPages.find(p =>
      candidates.some(c => p.filename === c || p.filename === `${c}`)
    )
    const filename = page ? page.filename : (slug.endsWith('.md') ? slug : `${slug}.md`)
    handleViewPage(filename, { fromLink: true })
