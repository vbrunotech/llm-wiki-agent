...
export default function IngestModal({ onClose, onSubmit, onSubmitFile, processing }) {
  ...
  return (
    ...
    >
      <div className="bg-surface border border-border-subtle rounded-2xl w-[600px] max-w-[95vw]
                      max-h-[85vh] flex flex-col shadow-2xl">
        {/* Header */}
        <div className="flex items-center justify-between px-6 py-4 border-b border-border">
          <h2 className="text-base font-semibold text-heading">Ingest Source</h2>
            ...
            <div className="space-y-4">
              <div className="space-y-1.5">
                ...
                <input
                  ...
                  className="w-full bg-raised border border-border-subtle rounded-lg px-4 py-2.5 text-sm
                             text-heading placeholder-faint focus:outline-none focus:border-blue-500
                             transition-colors"
                />
              </div>
              <div className="space-y-1.5">
                <label className="text-xs font-semibold text-muted uppercase tracking-wide">Content</label>
                <textarea
                  value={content}
                  onChange={e => setContent(e.target.value)}
                  placeholder="Paste the article, document, or notes here…"
                  rows={12}
                  className="w-full bg-raised border border-border-subtle rounded-lg px-4 py-3 text-sm
            ...
            <div className="space-y-4">
              ...
              >
                ...
                <p className="text-sm text-muted">
                  Drop a file here or <span className="text-link">browse</span>
                </p>
              ...
              {file && (
                <div className="flex items-center gap-3 px-4 py-3 bg-raised rounded-xl border border-border-subtle">
                  <FileText size={16} className="text-blue-400 shrink-0" />
                  <div className="flex-1 min-w-0">
                    <p className="text-sm text-heading truncate">{file.name}</p>
